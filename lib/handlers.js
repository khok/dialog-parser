...
async function arrayHandler (parameter, ui) {
	...
	ui.message('comma_sep', [parameter.text, values.length])
	const texts = (await ui.input()).replace(/\s/g, '').split(',');
...
async function textHandler(parameter, ui) {
	return await ui.input(parameter.text);
}
